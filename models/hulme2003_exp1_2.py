...
#    test = test.sample(frac=1)
    ...
    dat['duration'][(dat['procedure'] == 'test') & (dat['sp'] == 1)] = 1000
...
def gen_trials(n_subjects, nlists, r_int):
    ...
    for subject in np.arange(n_subjects):
        for listn in range(nlists):
            ...
            dat3 = gen_trial_list(3, 3, r_int, 4*listn+3, alternating='HFLF')
            dat4 = gen_trial_list(3, 3, r_int, 4*listn+4, alternating='LFHF')
            dat1['composition'] = 'Pure HF'
            dat2['composition'] = 'Pure LF'
            dat3['composition'] = 'Alternating HFLF'
            dat4['composition'] = 'Alternating LFHF'
...
#%%
data1 = pd.DataFrame(dict(sp = [1,2,3,4,5,6]*4,
                         composition = ['Pure HF']*6 + 
                                       ['Pure LF']*6 + 
                                       ['Alternating HFLF']*6 + 
                                       ['Alternating LFHF']*6,
                         freq = ['hf']*6 + ['lf']*6 + ['hf','lf']*3 + ['lf','hf']*3,
                         acc = [0.92,0.80,0.70,0.57,0.52,0.69,
                                0.86,0.68,0.57,0.40,0.25,0.57,
                                0.90,0.73,0.64,0.44,0.33,0.67,
                                0.86,0.70,0.61,0.44,0.33,0.61],
                         procedure = 'test',
                         experiment = 1))

data2 = pd.DataFrame(dict(sp = [1,2,3,4,5,6]*4,
                         composition = ['Pure HF']*6 + 
                                       ['Pure LF']*6 + 
                                       ['Alternating HFLF']*6 + 
                                       ['Alternating LFHF']*6,
                         freq = ['hf']*6 + ['lf']*6 + ['hf','lf']*3 + ['lf','hf']*3,
                         acc = [0.90,0.75,0.58,0.40,0.38,0.63,
                                0.82,0.60,0.37,0.24,0.20,0.50,
                                0.90,0.70,0.50,0.35,0.24,0.57,
                                0.83,0.68,0.47,0.33,0.27,0.52],
                         procedure = 'test',
                         experiment = 2))

data3 = pd.DataFrame(dict(sp = [1,2,3,4,5,6]*4,
...
model = sac.fit_model_group.Model(nets=nets, 
                        ...
                        par_ranges={'w_recovery_rate': slice(0.2,0.21,0.005),
                                                  'W': slice(5,7,0.2),
                        },              
                        equations=equations, 
                        group_vars=['freq','sp','composition'], 
...
results1.to_csv('output/final/hulme2003_simulation_exp1_2.csv')
model.save_pars('hulme2003')
