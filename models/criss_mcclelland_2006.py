...
def gen_trial_list(hf_num, lf_num, listn):
    ...
    lfwords = sac.utils.generate_words(lf_num, 0, 10)
    hfwords['freq'] = 'hf'
    lfwords['freq'] = 'lf'
    hfwords['duration'] = 0.15
    lfwords['duration'] = 0.15
    hfwords['duration'][int(hf_num/3):int(2*hf_num/3)] = 0.30
    lfwords['duration'][int(lf_num/3):int(2*lf_num/3)] = 0.30
    hfwords['duration'][int(2*hf_num/3):hf_num] = 0.60
    lfwords['duration'][int(2*lf_num/3):lf_num] = 0.60
#    study_idx = np.random.choice(np.arange(hf_num), replace=False, size=int(hf_num/2))
    study = pd.concat([hfwords,lfwords])
    study['duration'] = study['duration']+0.25
...
data = pd.DataFrame(dict(freq = ['hf','hf','hf','lf','lf','lf'],
                          study_duration = [0.15,0.30,0.60,0.15,0.30,0.600],
                          hits = [0.46,0.469,0.49,0.436,0.489,0.536],
                          procedure = 'test'))
...
model = sac.fit_model_group.Model(nets=nets, 
                        ...
                        par_ranges={'w_recovery_rate': slice(0.99,1.00,0.005),
                                                  'W': slice(2,3,0.2),
...
results1.to_csv('output/final/criss_mcclelland_2006.csv')
model.save_pars('criss_mcclelland_2006')
